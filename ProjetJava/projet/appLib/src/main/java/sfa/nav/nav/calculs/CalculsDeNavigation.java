...
public class CalculsDeNavigation {
	private static Logger logger = LoggerFactory.getLogger(CalculsDeNavigation.class);

	public CalculsDeNavigation() {
	}
	...
	public PointGeographique estimeLoxodromique(PointGeographique Depart, Cap routeFond, Distance d) {
		...
		if (d.distanceInMilleNautique() < 300) {
			...
			logger.error("Je n esai spas calculer au dela de 300 Mn");
			pg = null;
		}
		return pg;
	}
	...
	// --> cap NON consant
	// ------------------------------------------------------------------------------------------------
	public DataOrthoDromie capOrthodromique(PointGeographique Depart, PointGeographique Arrivee) throws NavException {
		Vitesse vitesseRoutefondInitiale = VitesseFactory.fromNoeud(0.0);
		return capOrthodromique(Depart, Arrivee, vitesseRoutefondInitiale, 0.0);
	}

	public DataOrthoDromie capOrthodromique(PointGeographique Depart, PointGeographique Arrivee,
			Vitesse vitesseRoutefondInitiale, double dureeDeNavEnHeureDecimale) throws NavException {
		DataOrthoDromie retour = new DataOrthoDromie();
		...
		/*
		 * === Distance orthodromique === l: latitude L: Longitude
		 * 
		 * Distance = arccos [ sin(LatA) * sin(LatB) + cos(LatA) * cos(LatB) * cos(LongB
		 * − LongA)]
		 * 
		 * Cap initial:
		 * 
		 * Cos (LatA) * Tan (LatB) Sin (LatA) CoTan (cap) = ----------------------------
		 * - ----------------------- Sin (LongB - LongA) Tan (LongB - Long A)
		 * 
		 * 
		 * si Meme longitude, on se deplace sur la latitude de bas en haut Distance =
		 * arccos [ sin(LatA) * sin(LatB) + cos(LatA) * cos(LatB)] = arccos [ cos(LatA -
		 * LatB) ] = LatA - LatB cap: 0 ou 180°
		 */

		double LatDepartAsRadian = Depart.latitude().asRadian();
		double LatArriveeAsRadian = Arrivee.latitude().asRadian();
		double LongiDepartAsRadian = Depart.longitude().asRadian();
		double LongiArriveeAsRadian = Arrivee.longitude().asRadian();

		double l = LatArriveeAsRadian - LatDepartAsRadian;
		double g = LongiArriveeAsRadian - LongiDepartAsRadian;
		double gOptimum = g;

		// sens du deplacement
		ePointsCardinaux[] sens = { ePointsCardinaux.N, ePointsCardinaux.E };

		sens[0] = ePointsCardinaux.N;
		if (l < 0)
			sens[0] = ePointsCardinaux.S;

		sens[1] = ePointsCardinaux.E;
		if (gOptimum < 0)
			sens[1] = ePointsCardinaux.W;
		...
		if (Math.abs(g) > Math.PI) {
			...
			logger.debug("Changement de sens: g {} gOptimun {} sens {}", AngleFactory.fromRadian(g),
					AngleFactory.fromRadian(gOptimum), sens[1].getTag());
		}

		// deplacement a Longitude constante
		if (Math.abs(gOptimum) <= Angle.DegreToRadian(1.0)) {
			logger.debug("Deplacement a longitude constante : delta longitude {}°",
					Math.abs(Angle.RadianToDegre(gOptimum)));

			retour.distance(DistanceFactory.fromKm(Math.abs(Constantes.RayonTerrestreEnKm * (l))));
			if (l < 0)
				retour.cap( /* vers sud */ CapFactory.fromDegre(180.0));
			else
				retour.cap(/* vers le nord */ CapFactory.fromDegre(0.0));
			retour.vertex(PointGeographiqueFactory.fromLatLong(
					LatitudeFactory.fromDegre((Depart.latitude().asDegre() + Arrivee.latitude().asDegre()) / 2.0),
					LongitudeFactory.fromDegre(Depart.longitude().asDegre())));
		} else {
			double cosM = Math.sin(LatDepartAsRadian) * Math.sin(LatArriveeAsRadian)
					+ Math.cos(LatDepartAsRadian) * Math.cos(LatArriveeAsRadian) * Math.cos(gOptimum);
			...
			// ---------------------------------------
			double cosAngleRouteInitiale = (Math.sin(LatArriveeAsRadian) - Math.sin(LatDepartAsRadian) * cosM)
					/ (Math.cos(LatDepartAsRadian) * sinM);
			...
			// ---------------------------------------
			double cosLatVertex = Math.cos(LatDepartAsRadian) * Math.sin(AngleRouteInitiale);
			double LatVertexAsRadian = Math.acos(cosLatVertex);
			Latitude latitudeVertex;
			if (Math.abs(AngleRouteInitiale) >= (Math.PI / 2.0))
				latitudeVertex = LatitudeFactory.fromDegreAndSens(LatVertexAsRadian * 180.0 / Math.PI,
						ePointsCardinaux.Sud);
			else
				latitudeVertex = LatitudeFactory.fromDegreAndSens(LatVertexAsRadian * 180.0 / Math.PI,
						ePointsCardinaux.Nord);

			double cos_gVertex = Math.tan(LatDepartAsRadian) / Math.tan(LatVertexAsRadian);
			double gVertexAsRadian = Math.acos(cos_gVertex);
			double LongiVertex = 0.0;
			/*
			 * ePointsCardinaux directionVersLeVertex = sens[1]; ePointsCardinaux
			 * sensLongiVertex = Depart.longitude().getSens(); if (gOptimum < 0) { //
			 * deplacement vers l'ouest gVertex = Math.abs(gVertex) * (-1.0);
			 * directionVersLeVertex = ePointsCardinaux.W; } else { gVertex =
			 * Math.abs(gVertex); directionVersLeVertex = ePointsCardinaux.E; }
			 * 
			 * LongiVertex = LongiDepart + gVertex; if (Math.abs(LongiVertex) > Math.PI) {
			 * if (directionVersLeVertex.equals(ePointsCardinaux.E)) { LongiVertex =
			 * LongiVertex -Math.PI; LongiVertex = Math.PI - LongiVertex; sensLongiVertex =
			 * sensLongiVertex.sensInverse(); } if
			 * (directionVersLeVertex.equals(ePointsCardinaux.W)) { LongiVertex =
			 * Math.abs(LongiVertex) -Math.PI; LongiVertex = Math.PI - LongiVertex;
			 * sensLongiVertex = sensLongiVertex.sensInverse(); } } else { sensLongiVertex =
			 * ePointsCardinaux.E; if (LongiVertex < 0) sensLongiVertex =
			 * ePointsCardinaux.Ouest; LongiVertex = Math.abs(LongiVertex); } logger.
			 * debug("Variation de longitude du vertex: gVertex {} sens {} LongitudeVertex {}"
			 * , AngleFactory.fromRadian(gVertex), directionVersLeVertex.getTag(),
			 * AngleFactory.fromRadian(LongiVertex));
			 */

			// --------------------------------------------------
			// la latitude du vertex est egale a 0 ==> on navigue forcement a latitude nulle
			// ==> Math.acos(Math.tan(LatDepartAsRadian) /
			// Math.tan(latitudeVertex.asRadian()) = acos(1) = 1
			// --------------------------------------------------
			if (Math.abs(latitudeVertex.asRadian()) < 0.01) {
				LongiVertex = LongiDepartAsRadian + gOptimum * (gOptimum / Math.abs(gOptimum)) * 1.0;
			} else {
				LongiVertex = LongiDepartAsRadian + (gOptimum / Math.abs(gOptimum))
						* Math.acos(Math.tan(LatDepartAsRadian) / Math.tan(latitudeVertex.asRadian()));
			}
			ePointsCardinaux SensLongitudeVertex = ePointsCardinaux.Est;
			if (LongiVertex < 0) {
				SensLongitudeVertex = ePointsCardinaux.Ouest;
				LongiVertex = Math.abs(LongiVertex);
			}

			if (LongiVertex > Math.PI) {
				// navigation d'Est en Ouest et le vertex est de l'autre cote que me meridien
				// 180°
				LongiVertex = 2 * Math.PI - LongiVertex;
				SensLongitudeVertex = SensLongitudeVertex.sensInverse();
			}

			Longitude longitudeVertex = LongitudeFactory.fromDegreAndSens(LongiVertex * 180.0 / Math.PI,
					SensLongitudeVertex);
			retour.vertex(PointGeographiqueFactory.fromLatLong(latitudeVertex, longitudeVertex));

			double V = routeOrthoAuDepart;
			double alphaEnDegre = ((vitesseRoutefondInitiale.vitesseInNoeud() * dureeDeNavEnHeureDecimale) / (120.0))
					* Math.sin(V) * Math.tan(LatDepartAsRadian);
			double alphaRad = alphaEnDegre * Math.PI / 180.0;

			Cap rfInitaile = CapFactory.fromDegre((V + alphaRad) * 180.0 / Math.PI);
			retour.Rfinitiale(rfInitaile);
		}
	...
	// --> cap constant
	// ------------------------------------------------------------------------------------------------
	public DataLoxodromieCapDistance capLoxodromique(PointGeographique Depart, PointGeographique Arrivee) {
		...
		/*
		 * CA c'est le cas de tous les jours, la vrai nav .... !! --> Cette formule
		 * n'est valable que si la différence des longitudes est inférieure à 180°
		 */
		// le mieux explique:
		// http://serge.mehl.free.fr/anx/loxodromie.html
		 ...
		 * Pi x Guderman (x) = -- + --- 4 2
		 * 
		 * LongiArrivee - LongiDepart Direction = ArcTan (
		 * ------------------------------------------------------------ ) ln( tan(
		 * Guderman( LatArrivee )) - ln( tan( Guderman( LatDepart ))
		 * 
		 * LatArrivee - LatDepart Distance = ----------------- cos ( Cap )
		 */
		...
		if (Math.abs(LatDepart - LatArrivee) < Angle.DegreToRadian(0.01)) {
			logger.debug("Deplacement a latitude constante : delta latitude {}°",
					Math.abs(Angle.RadianToDegre(LatDepart - LatArrivee)));
			...
			retour.distance(
					DistanceFactory.fromKm(Constantes.RayonTerrestreEnKm * Math.abs(gOptimum * Math.cos(LatDepart))));
		} else {
			...
			if (LatDepart < 0)
				LatitudeCroissanteDepart = LatitudeCroissanteDepart * (-1.0);

			double LatitudeCroissanteArrivee = Math.abs(Math.log(Math.tan(GuderMannArrivee)));
			if (LatArrivee < 0)
				LatitudeCroissanteArrivee = LatitudeCroissanteArrivee * (-1.0);
			...
			// Pour résoudre certains calculs particuliers (Routes et distances
			// loxodromiques), on utilise la notion de route fond par quart.
			// Dans ce cas la route est exprimée en degrés de 0° à 90° en partant du nord ou
			// du sud puis
			...
			// -------------------------------
			double RF = 0.0;
			if (sens[0].equals(ePointsCardinaux.N)) {
				if (sens[1].equals(ePointsCardinaux.E))
					RF = 0.0 * Math.PI + RFQ; /* N - E */
				else
					RF = 2.0 * Math.PI - RFQ; /* N - W */
			} else {
				if (sens[1].equals(ePointsCardinaux.E))
					RF = 1.0 * Math.PI - RFQ; /* S - E */
				else
					RF = 1.0 * Math.PI + RFQ; /* S - W */
			}

			retour.cap(CapFactory.fromRadian(RF));
			retour.distance(DistanceFactory
					.fromKm(Constantes.RayonTerrestreEnKm * (Math.abs(LatArrivee - LatDepart)) / Math.cos(RFQ)));
		}
