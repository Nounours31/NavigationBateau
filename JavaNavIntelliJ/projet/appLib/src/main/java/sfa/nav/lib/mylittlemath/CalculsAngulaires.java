...
import sfa.nav.lib.model.Angle;
import sfa.nav.lib.model.Cap;
import sfa.nav.lib.model.Distance;
...
public class CalculsAngulaires {
	...
	private void capOrthodromique(PointGeographique A, PointGeographique B, HandlerOnCapDistance retour) throws NavException {
		...
		*/
		...
		if (Math.abs(B.longitude().asDegre() - A.longitude().asDegre()) <= 1.0) {
			...
			logger.debug("\tAngle solide entre A et B: {}", Angle.fromRadian(angleSpheriqueEntreAetB));
			...
			logger.debug("\tDirection entre A et B: {}",Angle.fromRadian(directionOrthodromiqueEntreLesPoints));

			
			retour._cap = Cap.fromAngle(Angle.fromRadian(directionOrthodromiqueEntreLesPoints));
		...
		if (retour._distance.distanceInKm() < 0.0) {
			...
			retour._cap = Cap.fromDegre(retour._cap.asDegre() + 180.0);
	...
	private void capLoxodromique(PointGeographique A, PointGeographique B, HandlerOnCapDistance retour) throws NavException {
		...
		if (Math.abs(A.latitude().asDegre() - B.latitude().asDegre()) < 1.0) {
			...
			retour._cap = Cap.fromDegre(90.0);
			if (g < 0)
				retour._cap = Cap.fromDegre(270.0);
			...
			logger.debug("\tRFQ entre A et B: {}", Angle.fromRadian(RFQ));
