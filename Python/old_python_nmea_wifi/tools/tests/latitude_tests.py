import pytest

from tools.angle import angle
from tools.latitude import latitude




class latitude_tests:
    def test_init(self):
        l : latitude = latitude(0.0)
        assert l.toString(base=angle.STR_AsMin, detail=angle.DISPLAY_SHORT) == "N 000°00.000'"

    def test_fromstring_dd(self):
        x = "-47.12"
        l: latitude = latitude.fromString(x)
        assert l.toString(base=angle.STR_AsMin, detail=angle.DISPLAY_SHORT) == "S 047°07.200'"

    def test_fromstring_dd2(self):
        x = "S 47.12"
        l: latitude = latitude.fromString(x)
        assert l.toString(base=angle.STR_AsMin, detail=angle.DISPLAY_SHORT) == "S 047°07.200'"

    def test_fromstring_dd3(self):
        x = "S 47°07.200"
        l: latitude = latitude.fromString(x)
        assert l.toString(base=angle.STR_AsMin, detail=angle.DISPLAY_SHORT) == "S 047°07.200'"

    def test_fromstring_dd4(self):
        x = "N 47°07.200"
        l: latitude = latitude.fromString(x)
        assert l.toString(base=angle.STR_AsMin, detail=angle.DISPLAY_SHORT) == "N 047°07.200'"

    def test_fromstring_dd5(self):
        x = "47°07.200"
        l: latitude = latitude.fromString(x)
        assert l.toString(base=angle.STR_AsMin, detail=angle.DISPLAY_SHORT) == "N 047°07.200'"

    def test_fromstring_dd6(self):
        x = "-47°07.200"
        l: latitude = latitude.fromString(x)
        assert l.toString(base=angle.STR_AsMin, detail=angle.DISPLAY_SHORT) == "S 047°07.200'"

    def test_fromstring_dd7(self):
        x = "47.12"
        l: latitude = latitude.fromString(x)
        assert l.toString(base=angle.STR_AsMin, detail=angle.DISPLAY_SHORT) == "N 047°07.200'"

    def test_fromstring_dd8(self):
        x = "-47.12"
        l: latitude = latitude.fromString(x)
        assert l.toString(base=angle.STR_AsMin, detail=angle.DISPLAY_SHORT) == "S 047°07.200'"




