...
class cAstro ():    
    ...
    def paralaxe(horizontaleParalaxeEnDeg : float, hauteurObservationEnDeg : float) -> float :
        return math.asin (math.sin(horizontaleParalaxeEnDeg * __DEG_2_RAD__) * math.cos(hauteurObservationEnDeg * __DEG_2_RAD__)) * __RAD_2_DEG__

    def refraction(hauteurObservationEnDeg : float, *, temperatureEnDeg : float = 10.0, pressionEnHPa : float = 1010.0) -> float :
        r0 : float = 1 / math.tan ((hauteurObservationEnDeg + (7.31 / (hauteurObservationEnDeg + 4.4))) * __DEG_2_RAD__)
