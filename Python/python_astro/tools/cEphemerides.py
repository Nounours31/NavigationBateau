...
import math
from skyfield.api import Angle, Star, load, wgs84, N, E, W, S
from skyfield import api, toposlib
...
class cEphemerides:
    ...
    def planete_info(d : datetime, eph: any):        # used in starstab
        ...
        for planete in cEphemerides.planete_db:
            ...
            dist_km = distance.km
            semiDiametre  = (math.atan(planete_rayon / dist_km)) * 180.0 / math.pi   # volumetric mean radius of sun = 695700 km
            print (f"{planete_humannom:s} - GHA: {gha} - Dec: {dec} - SD: {cAngle.toStringDebug(semiDiametre)}\n")
            ...
            print (f"{planete_humannom:s} - azimut: {cEphemerides.fmtdeg(az.degrees)} - Dec: {cEphemerides.fmtdeg(alt.degrees)} - HP: {cEphemerides.fmtdeg(HP)} - DEC+HP: {cEphemerides.fmtdeg(alt.degrees+HP)}\n")
