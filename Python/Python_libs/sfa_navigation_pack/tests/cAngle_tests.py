...
class cAngle_tests:
    ...
    def test_addition(self):
        ...
        l3: cAngle = l1 + l2
        assert l3.valAsDeg == pytest.approx(90.0, cAngle.EQUAL_TOLERANCE_IN_DEG)

        l3 = l3 + 90.0
        assert l3.valAsDeg == pytest.approx(180.0, cAngle.EQUAL_TOLERANCE_IN_DEG)

        l4 = 90.0 + l3
        assert l3.valAsDeg == pytest.approx(180.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
        assert l4.valAsDeg == pytest.approx(270.0, cAngle.EQUAL_TOLERANCE_IN_DEG)


        l3 += l1
        assert l3.valAsDeg == pytest.approx(225.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
        assert l1.valAsDeg == pytest.approx(45.0, cAngle.EQUAL_TOLERANCE_IN_DEG)

        l3 += 45.0
        assert l3.valAsDeg == pytest.approx(270.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
    ...
    def test_substarct(self):
        ...
        l3: cAngle = l1 - l2
        assert l3.valAsDeg == pytest.approx(0.0, cAngle.EQUAL_TOLERANCE_IN_DEG)

        l3 = l3 + 90.0
        l3 = l3 - 45.0
        assert l3.valAsDeg == pytest.approx(45.0, cAngle.EQUAL_TOLERANCE_IN_DEG)

        l4 = 45.0 - l3
        assert l3.valAsDeg == pytest.approx(45.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
        assert l4.valAsDeg == pytest.approx(0.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
    ...
    def test_multi(self):
        ...
        l4: cAngle = (-1.0) * l1
        assert l4.valAsDeg == pytest.approx(-45.0, cAngle.EQUAL_TOLERANCE_IN_DEG)

        l5: cAngle = l1 * l2
        assert l5.valAsDeg == pytest.approx(2025.0, cAngle.EQUAL_TOLERANCE_IN_DEG)

        l4 *= 10.0
        assert l4.valAsDeg == pytest.approx(-450.0, cAngle.EQUAL_TOLERANCE_IN_DEG)

        l3 *= l4
        assert l3.valAsDeg == pytest.approx(20250.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
        assert l4.valAsDeg == pytest.approx(-450.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
    ...
    def test_divise(self):
        ...
        l3: cAngle = l1 / 2.0
        assert l3.valAsDeg == pytest.approx(22.5, cAngle.EQUAL_TOLERANCE_IN_DEG)

        l3: cAngle = l1 / (-1.0)
        assert l3.valAsDeg == pytest.approx(-45.0, cAngle.EQUAL_TOLERANCE_IN_DEG)

        l4: cAngle = (-1.0) / l1
        assert l4.valAsDeg == pytest.approx(-0.02222222222222222, cAngle.EQUAL_TOLERANCE_IN_DEG)
        assert l1.valAsDeg == pytest.approx(45.0, cAngle.EQUAL_TOLERANCE_IN_DEG)

        l5: cAngle = l1 / l2
        assert l5.valAsDeg == pytest.approx(1.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
        assert l1.valAsDeg == pytest.approx(45.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
        ...
        l4 *= 10.0
        l3 /= l4
        assert l3.valAsDeg == pytest.approx(-4.50, cAngle.EQUAL_TOLERANCE_IN_DEG)
        assert l4.valAsDeg == pytest.approx(10.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
    ...
    def test_normalise(self):
        ...
        l3: cAngle = l1 * 100.0
        assert l3.valAsDeg == pytest.approx(4500.0, cAngle.EQUAL_TOLERANCE_IN_DEG)

        l2 = l3.normalise()
        assert l3.valAsDeg == pytest.approx(4500.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
        assert l2.valAsDeg == pytest.approx(180.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
        ...
        l2 = cAngle(361.0)
        l2 = l2.normalise()
        assert l2.valAsDeg == pytest.approx(1.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
        ...
        l2 = cAngle(-1.0)
        l2 = l2.normalise()
        assert l2.valAsDeg == pytest.approx(359.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
        l2 = cAngle(-1.0)
        l2.inner_normalise()
        assert l2.valAsDeg == pytest.approx(359.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
        ...
        l2 = cAngle(-500.0)
        l2 = l2.normalise()
        assert l2.valAsDeg == pytest.approx(220.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
        l2 = cAngle(-500.0)
        l2.inner_normalise()
        assert l2.valAsDeg == pytest.approx(220.0, cAngle.EQUAL_TOLERANCE_IN_DEG)
